...
def GetTeams(season, league):
	...
	strSearch = 'SELECT team_id, team_name, league FROM TeamStatistics WHERE league = \"' + league + '\" Group By team_id ORDER BY team_id ASC'
...
def GetPlayers(season, teamID):
	
	fpDatabase = Global.Dir_Root + season + '\\' + Global.Fn_Database
	conn = sqlite3.connect(fpDatabase)
	cursorObj = conn.cursor()
	
	strSearch = 'SELECT player_id, player_name, team_id FROM PlayerStatistics WHERE team_id = ' + str(teamID) + ' Group By player_id ORDER BY player_id ASC'
	cursorObj.execute(strSearch)
